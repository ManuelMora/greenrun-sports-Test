...
export default class CountryService {
    public static createCountry(country: ICountry): Promise<IResponse> {
        return new Promise(async (resolve, reject) => {
            try {
                await databaseClient<ICountry>('countries').insert(country);
    ...
    public static updateCountry(country: ICountry): Promise<IResponse> {
        return new Promise(async (resolve, reject) => {
            try {
                await databaseClient<ICountry>('countries').where({ id: country.id }).update(country);
    ...
    public static getCountries(): Promise<IResponse> {
        return new Promise(async (resolve, reject) => {
            try {
                const countriesList = await databaseClient<ICountry>('countries').where({ country_status: true }).select();
    ...
    public static getCountryById(id: number): Promise<IResponse> {
        return new Promise(async (resolve, reject) => {
            try {
                const countryMatch = await databaseClient<ICountry>('countries').where({ id }).select().limit(1);
    ...
    public static deleteCountry(id: number): Promise<IResponse> {
        return new Promise(async (resolve, reject) => {
            try {
                await databaseClient<ICountry>('countries').where({ id }).update({ country_status: false });
